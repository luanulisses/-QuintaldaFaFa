...
Deno.serve(async (req) => {
  ...
  try {
    const { purchase_id, payment_id } = await req.json();

    if (!purchase_id && !payment_id) {
      throw new Error("purchase_id ou payment_id é obrigatório");
    }
    ...
    let purchase;
    if (purchase_id) {
      const { data, error } = await supabase.from("arraia_purchases").select("*").eq("id", purchase_id).single();
      if (error || !data) throw new Error("Compra não encontrada pelo ID.");
      purchase = data;
    } else {
      const { data, error } = await supabase.from("arraia_purchases").select("*").eq("payment_id", payment_id).single();
      if (error || !data) throw new Error("Compra não encontrada pelo Payment ID.");
      purchase = data;
    }
    ...
    if (!purchase.payment_id) {
      throw new Error("Compra não possui payment_id salvo. Não é possível verificar no MP.");
    }
    ...
    if (payment.status !== "approved") {
      return new Response(JSON.stringify({ success: false, status: payment.status }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
